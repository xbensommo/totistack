<template>
  <CmsPageShell title="Redirects" description="Path redirects for slug changes and content migrations.">
    <template #actions>
...
const collectionName = 'cmsRedirects'
const columns = [{ key: 'fromPath', label: 'From' }, { key: 'toPath', label: 'To' }, { key: 'statusCode', label: 'Code' }, { key: 'status', label: 'Status' }]
const search = ref('')
...
const metrics = computed(() => [{ label: 'Redirects', value: items.value.length }, { label: 'Active', value: items.value.filter((i) => i.status === 'active').length }, { label: 'Inactive', value: items.value.filter((i) => i.status !== 'active').length }])
