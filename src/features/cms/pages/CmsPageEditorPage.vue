<template>
  <CmsPageShell title="Page editor" description="Editor surface for draft blocks, SEO fields, and publish state checks.">
    <template #actions>
...
const columns = [{ key: 'title', label: 'Title' }, { key: 'slug', label: 'Slug' }, { key: 'path', label: 'Path' }, { key: 'status', label: 'Status' }, { key: 'updatedAt', label: 'Updated' }]
const search = ref('')
...
const metrics = computed(() => [{ label: 'Loaded pages', value: items.value.length }, { label: 'Published', value: items.value.filter((i) => i.status === 'published').length }, { label: 'Drafts', value: items.value.filter((i) => i.status === 'draft').length }])
