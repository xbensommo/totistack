<template>
  <CmsPageShell title="Entries" description="Structured content entries with publishable versions.">
    <template #actions>
...
const collectionName = 'cmsEntries'
const columns = [{ key: 'title', label: 'Entry' }, { key: 'slug', label: 'Slug' }, { key: 'status', label: 'Status' }, { key: 'publishedAt', label: 'Published' }, { key: 'updatedAt', label: 'Updated' }]
const search = ref('')
...
const metrics = computed(() => [{ label: 'Entries', value: items.value.length }, { label: 'Published', value: items.value.filter((i) => i.status === 'published').length }, { label: 'Drafts', value: items.value.filter((i) => i.status === 'draft').length }])
