<template>
  <CmsPageShell title="Content types" description="Schema definitions for structured entries.">
    <template #actions>
...
const collectionName = 'cmsContentTypes'
const columns = [{ key: 'name', label: 'Name' }, { key: 'slug', label: 'Slug' }, { key: 'status', label: 'Status' }, { key: 'updatedAt', label: 'Updated' }]
const search = ref('')
...
const metrics = computed(() => [{ label: 'Types', value: items.value.length }, { label: 'Active', value: items.value.filter((i) => i.status === 'active').length }, { label: 'Fields', value: items.value.reduce((sum, i) => sum + (Array.isArray(i.fields) ? i.fields.length : 0), 0) }])
