<template>
  <CmsPageShell title="Pages" description="Draft/edit cheaply; publish through declared server actions.">
    <template #actions>
...
const columns = [{ key: 'title', label: 'Title' }, { key: 'path', label: 'Path' }, { key: 'status', label: 'Status' }, { key: 'publishedAt', label: 'Published' }, { key: 'updatedAt', label: 'Updated' }]
const search = ref('')
...
const metrics = computed(() => [{ label: 'Pages', value: items.value.length }, { label: 'Scheduled', value: items.value.filter((i) => i.status === 'scheduled').length }, { label: 'Archived', value: items.value.filter((i) => i.status === 'archived').length }])
