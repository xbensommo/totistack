<template>
  <CmsPageShell title="Menus" description="Site navigation structures kept separate from pages.">
    <template #actions>
...
const collectionName = 'cmsMenus'
const columns = [{ key: 'name', label: 'Name' }, { key: 'handle', label: 'Handle' }, { key: 'status', label: 'Status' }, { key: 'updatedAt', label: 'Updated' }]
const search = ref('')
...
const metrics = computed(() => [{ label: 'Menus', value: items.value.length }, { label: 'Active', value: items.value.filter((i) => i.status === 'active').length }, { label: 'Items', value: items.value.reduce((sum, i) => sum + (Array.isArray(i.items) ? i.items.length : 0), 0) }])
