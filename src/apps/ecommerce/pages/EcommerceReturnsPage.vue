<template>
  <CommercePageShell eyebrow="Commerce" title="Returns" description="Approve or reject returns with clean audit and notification hooks.">
    <template #actions>
...
const collectionName = 'commerceReturns'
const columns = [{ key: 'orderId', label: 'Order' }, { key: 'status', label: 'Status' }, { key: 'requestedAmount', label: 'Requested' }, { key: 'approvedAmount', label: 'Approved' }, { key: 'createdAt', label: 'Created' }]
...
const metrics = computed(() => [{ label: 'Returns', value: items.value.length }, { label: 'Pending', value: items.value.filter((i) => i.status === 'pending').length }, { label: 'Approved', value: items.value.filter((i) => i.status === 'approved').length }, { label: 'Rejected', value: items.value.filter((i) => i.status === 'rejected').length }])
