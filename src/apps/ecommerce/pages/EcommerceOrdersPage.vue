<template>
  <CommercePageShell eyebrow="Commerce" title="Orders" description="Control order state without hiding business decisions inside components.">
    <template #actions>
...
const columns = [{ key: 'orderNumber', label: 'Order' }, { key: 'status', label: 'Status' }, { key: 'paymentStatus', label: 'Payment' }, { key: 'fulfillmentStatus', label: 'Fulfillment' }, { key: 'total', label: 'Total' }, { key: 'createdAt', label: 'Created' }]
...
const metrics = computed(() => [{ label: 'Orders', value: items.value.length }, { label: 'Open', value: items.value.filter((i) => !['cancelled','delivered'].includes(i.status)).length }, { label: 'Paid', value: items.value.filter((i) => i.paymentStatus === 'paid').length }, { label: 'Total value', value: normalizeMoney(items.value.reduce((sum, i) => sum + Number(i.total || 0), 0), 'NAD') }])
