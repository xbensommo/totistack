<template>
  <CommercePageShell eyebrow="Commerce" title="Inventory" description="Stock ledger and manual adjustment visibility.">
    <template #actions>
...
const collectionName = 'commerceInventoryMovements'
const columns = [{ key: 'productId', label: 'Product' }, { key: 'movementType', label: 'Type' }, { key: 'quantity', label: 'Qty' }, { key: 'reason', label: 'Reason' }, { key: 'createdAt', label: 'Created' }]
...
const metrics = computed(() => [{ label: 'Movements', value: items.value.length }, { label: 'Adjustments', value: items.value.filter((i) => i.movementType === 'adjustment').length }, { label: 'Sales', value: items.value.filter((i) => i.movementType === 'sale').length }, { label: 'Restocks', value: items.value.filter((i) => i.movementType === 'restock').length }])
