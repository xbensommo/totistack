<template>
  <CommercePageShell eyebrow="Commerce" title="Store Settings" description="Manage storefront identity and operational defaults.">
    <template #actions>
...
const collectionName = 'commerceStores'
const columns = [{ key: 'name', label: 'Store' }, { key: 'slug', label: 'Slug' }, { key: 'status', label: 'Status' }, { key: 'currency', label: 'Currency' }, { key: 'updatedAt', label: 'Updated' }]
...
const metrics = computed(() => [{ label: 'Stores', value: items.value.length }, { label: 'Active', value: items.value.filter((i) => i.status === 'active').length }, { label: 'Draft', value: items.value.filter((i) => i.status === 'draft').length }, { label: 'Archived', value: items.value.filter((i) => i.status === 'archived').length }])
